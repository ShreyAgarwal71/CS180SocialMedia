...
public class ClientSideStorage {
	...
	private static Path getOSDataBasePath() {
		...
		if (os.contains("win")) {
			return Paths.get(System.getenv("APPDATA"), App.ID, FILE_NAME);
		} else if (os.contains("mac")) {
			return Paths.get(System.getProperty("user.home"), "/Library/Application Support", App.ID, FILE_NAME);
		} else if (os.contains("nix") || os.contains("nux")) {
			return Paths.get(System.getProperty("user.home"), String.format(".%s", App.ID.toLowerCase()), FILE_NAME);
		} else {
			return Paths.get(System.getProperty("user.home"), App.ID, FILE_NAME);
		}
	}
}
